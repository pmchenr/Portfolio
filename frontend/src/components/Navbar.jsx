...
export default function Navbar() {
  ...
  const linkClass = ({ isActive }) =>
    `px-1 hover:text-ink underline-offset-4 ${isActive ? "underline text-ink" : ""}`;

  return (
    <header className={`fixed inset-x-0 top-0 z-50 ${isHome ? "bg-transparent border-none" : "bg-white/80 backdrop-blur border-b border-line"}`}>
